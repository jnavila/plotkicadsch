...
let create_test parser checker =
 fun parse_expr expected_value ->
  ...
  match t with
  | Ok res ->
      checker expected_value res
  | Error e ->
      assert_failure ("not parsed: " ^ e)
...
let string_tests =
  test_list test_string
    ...
    ; ("(test  \"toto\")", "toto") (* white spacLKes in middle *)
...
let test_int () =
  ...
  match t with Ok 123 -> () | _ -> assert_failure "not parsed"
...
let at_tests = test_list test_at [("(at 0 0 0)", Coord (0, 0))]
...
let font_tests =
  test_list test_font
    ...
    ; ( "(font (size 1.7526 1.7526) italic bold)"
      , {font= None; size= Coord (69, 69); italic= true; bold= true; kolor= None}
      )
        ...
        ; effects=
            Some
              { font=
                  ...
                  ; bold= false
                  ; kolor= None }
              ; justify= Some {horiz= J_left; vert= J_bottom}
              ; hide= false } } )
    ; ( "(property \"Reference\" \"U\" (id 0) (at -4.6228 10.0584 0))"
      , { name= "Reference"
        ; value= "U"
...
let circle_tests =
  test_list test_circle
    ...
    ; ( {|(circle (center 0 1.27) (radius 1.27))
        |}
      , Circle (1, {center= RelCoord (0, 50); radius= 50}) ) ]
