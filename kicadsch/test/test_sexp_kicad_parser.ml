...
let create_test parser checker =
 fun parse_expr expected_value ->
  ...
  match t with
  | Ok (res, pos) -> (
    match Decode.run_with_result parser res with
    | Ok r ->
        checker expected_value r
    | Error sub -> (
      match Parsexp.Positions.find_sub_sexp_phys pos res ~sub with
      | Some err_range ->
          assert_failure
            (Format.sprintf "%d:%d: Decode failed for %s"
               err_range.start_pos.line err_range.start_pos.col
               (Sexplib0.Sexp.to_string sub) )
      | None ->
          assert_failure "decode failed!" ) )
  | Error e ->
      assert_failure ("not valid sexp " ^ Parsexp__Parse_error.message e)

let test_list single_test test_list =
  List.map ~f:(fun (expr, j) -> expr >:: fun _ -> single_test expr j) test_list

let check_yesno e1 e2 =
  assert_bool "yesno no match" ((e1 && e2) || ((not e1) && not e2))

let test_yesno = create_test (yesno_expr "foo") check_yesno

let yesno_tests =
  test_list test_yesno
    [({|(foo yes) |}, true); ({|(foo no)|}, false); ({|(foo) |}, true)]
...
let make_uuid u =
  ...
  | Some uuid2 ->
      uuid2
  | None ->
      raise (Invalid_argument ("internal error " ^ u))
...
let uuid_tests =
  test_list test_uuid
    [ ( {|(uuid 072ad1ed-9172-426c-8e5d-41f1dcd4b625)|}
      , make_uuid "072ad1ed-9172-426c-8e5d-41f1dcd4b625" ) ]
...
let color_tests =
  test_list test_kolor
    [ ("(color 1 2 3 4)", {red= 1; green= 2; blue= 3; alpha= 4.})
    ; ("(color 4 3 2 1)", {red= 4; green= 3; blue= 2; alpha= 1.}) ]

let check_at =
 fun (Coord (x1, y1), a1) (Coord (x2, y2), a2) ->
  assert_equal x1 x2 ; assert_equal y1 y2 ; assert_equal a1 a2
...
let check_paper =
 fun (Coord (x1, y1)) (Coord (x2, y2)) ->
  assert_equal ~printer:string_of_int x1 x2 ;
  assert_equal ~printer:string_of_int y1 y2

let test_paper = create_test paper_expr check_paper

let paper_tests =
  test_list test_paper [({|  (paper "A4") |}, Coord (29700, 21000))]
...
let fill_tests =
  test_list test_fill
    ...
    ; ( "(fill (type background))"
      , {fill_type= Some Background_fill; kolor= None} )
    ; ( "(fill (color 0 1 2 3.0))"
      , {fill_type= None; kolor= Some {red= 0; green= 1; blue= 2; alpha= 3.}} )
    ]

let check_justif =
 fun j {horiz; vert} -> assert_equal horiz j.horiz ; assert_equal vert j.vert

let test_justif = create_test justify_expr check_justif

let justif_tests =
  test_list test_justif
...
let check_font f {font; size; italic; bold; kolor} =
  ...
  | None, None ->
      ()
  | None, Some _ ->
      assert_failure "font name not found"
  | Some _, None ->
      assert_failure "spurious font name"
  | Some n1, Some n2 ->
      assert_equal n1 n2
...
let font_tests =
  test_list test_font
    [ ( "(font (size 1.7526 1.7526))"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= false
        ; bold= false
        ; kolor= None } )
    ; ( "(font (size 1.7526 1.7526) italic)"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= true
        ; bold= false
        ; kolor= None } )
    ; ( "(font (size 1.7526 1.7526) bold)"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= false
        ; bold= true
        ; kolor= None } )
    ; ( "(font (size 1.7526 1.7526) italic bold)"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= true
        ; bold= true
        ; kolor= None } )
    ; ( "(font (size 1.7526 1.7526) italic bold (color 0 0 0 0))"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= true
        ; bold= true
        ; kolor= Some {red= 0; green= 0; blue= 0; alpha= 0.} } )
    ; ( "(font (size 1.7526 1.7526)    (color 0 0 0 0))"
      , { font= None
        ; size= Coord (175, 175)
        ; italic= false
        ; bold= false
...
let property_tests =
  List.map
    ~f:(fun (expr, p) -> expr >:: fun _ -> test_property expr p)
    [ ( "(property \"Reference\" \"U\" (id 0) (at -4.6228 10.0584 0)\n\
        \        (effects (font (size 1.7526 1.7526)) (justify left bottom)))"
      , { name= "Reference"
        ; value= "U"
        ; id= 0
        ; at= Coord (-182, 396)
        ...
        ; effects=
            Some
              { font=
                  ...
                  ; bold= false
                  ; kolor= None }
              ; justify= Some {horiz= Some J_left; vert= Some J_bottom}
              ; hide= false } } )
    ; ( "(property \"Reference\" \"U\" (id 0) (at -4.6228 10.0584 0))"
      , { name= "Reference"
        ; value= "U"
    ...
    ; ( {|    (property "Références Inter-Feuilles" "${INTERSHEET_REFS}" (id 0) (at 71.6583 87.5506 0)
      ...
      , { name= "Références Inter-Feuilles"
        ; value= "${INTERSHEET_REFS}"
        ; id= 0
        ; at= Coord (716583, 875506)
        ; rot= 0
        ; effects=
            Some
              { font=
...
let assert_array_equal points1 points2 =
  List.iter
    ~f:(fun (RelCoord (x1, y1), RelCoord (x2, y2)) ->
      assert_equal ~cmp:Int.equal ~printer:string_of_int x1 x2 ;
      assert_equal ~cmp:Int.equal ~printer:string_of_int y1 y2 )
    (List.combine points1 points2)

let assert_list_equal list1 list2 =
  List.iter
    ~f:(fun (elt1, elt2) -> assert_equal elt1 elt2)
...
let check_rectangle shape1 shape2 =
  ...
  | _, _ ->
      assert_failure "rectangle should be a polygon"

let test_rectangle = create_test rectangle_prim_expr check_rectangle

let rectangle_tests =
  test_list test_rectangle
    ...
    ; ( "(rectangle (start 100.33 153.67) (end 106.68 160.02)\n\
      ...
      , Polygon
          ( 0
          , [ RelCoord (10033, 15366)
            ; RelCoord (10033, 16002)
            ; RelCoord (10668, 16002)
            ; RelCoord (10668, 15366)
            ; RelCoord (10033, 15366) ] ) ) ]

let check_pins p1 p2 =
  match (p1, p2) with
  | ( Pin {name; number; length; contact= RelCoord (x1, y1); orient}
    , Pin
        { name= name1
        ; number= number1
        ; length= length1
        ; contact= RelCoord (x2, y2)
        ; orient= orient1 } ) ->
  ...
  | _, _ ->
      assert_failure "pins should be Pins"
...
|}
    ...
    ; ( {|	(pin open_collector line
      ...
      , Pin
          ...
          ; number= ("3", Size 127)
          ; length= Size 254
          ; contact= RelCoord (1016, 508)
          ; orient= P_L } ) ]

let check_angles (as1, ae1) (as2, ae2) =
  assert_equal as1 as2 ; assert_equal ae1 ae2

let test_angles = create_test angles_expr check_angles

let angles_tests =
  test_list test_angles [({|(angles -180.0 0.0) |}, (-180.0, 0.0))]

let check_radius (RelCoord (x1, y1), length1, (as1, ae1))
    (RelCoord (x2, y2), length2, (as2, ae2)) =
...
let check_arcs a1 a2 =
  ...
  | ( Arc
        ...
        ; sp= RelCoord (xs1, ys1)
        ; ep= RelCoord (xe1, ye1)
        ; center= RelCoord (xc1, yc1) }
    , Arc
        { s= Size s2
        ; radius= radius2
        ; sp= RelCoord (xs2, ys2)
        ; ep= RelCoord (xe2, ye2)
        ; center= RelCoord (xc2, yc2) } ) ->
  ...
  test_list test_arc
    ...
    ; ( {|(arc (start -1.016 1.016) (end -1.016 -1.016) (radius (at -1.016 0) (length 1.016) (angles 90.1 -90.1))
      ...
      , Arc
          ...
          ; sp= RelCoord (-101, 101)
          ; ep= RelCoord (-101, -101)
          ; center= RelCoord (-101, 0) } ) ]

let check_bezier c1 c2 =
  match (c1, c2) with
  | Bezier (w1, points1), Bezier (w2, points2) ->
      assert_equal w1 w2 ;
      assert_array_equal points1 points2
  | _, _ ->
      failwith "must be a Bezier!"

let test_bezier = create_test bezier_expr check_bezier

let bezier_tests =
  test_list test_bezier
    [ ( {|(gr_curve
          (pts
            (xy 1.27 2.54)
            (xy 0.86 2.54)
...
let check_circles c1 c2 =
  match (c1, c2) with
  | ( Circle (w1, {center= RelCoord (x1, y1); radius= radius1})
    , Circle (w2, {center= RelCoord (x2, y2); radius= radius2}) ) ->
      assert_equal w1 w2 ;
      assert_equal x1 x2 ;
      assert_equal y1 y2 ;
      assert_equal radius1 radius2
  | _, _ ->
      failwith "must be a circle!"

let test_circle = create_test circle_expr check_circles

let circle_tests =
  test_list test_circle
...
let check_text c1 c2 =
  match (c1, c2) with
  | ( Text {c= RelCoord (x1, y1); text= t1; s= Size s1}
    , Text {c= RelCoord (x2, y2); text= t2; s= Size s2} ) ->
      assert_equal ~msg:"x1" x1 x2 ;
      assert_equal ~msg:"y1" y1 y2 ;
      assert_equal ~msg:"t1" t1 t2 ;
      assert_equal ~msg:"s1" s1 s2
  | _, _ ->
      failwith "must be a Text!"
...
let check_unit u1 u2 = assert_equal (List.length u1) (List.length u2)
...
|}
    ...
    ; ( {|(symbol "Conn_01x03_Male_1_1"
        (rectangle (start 0.8636 -2.413) (end -0.8636 2.413)))|}
      , [Field] )
    ; ({|(symbol "Conn_01x03_Male_1_1") |}, []) ]
...
let junction_tests =
  test_list test_junction
    [ ( "(junction (at 254 0) (diameter 0.9144) (color 0 0 0 0))"
      , Coord (25400, 0) )
...
let no_connect_tests =
  test_list test_no_connect
    [ ( "(no_connect (at 254 0) (uuid e8352a79-c40d-4cf1-97a7-2bb649ced79a))"
      , Coord (25400, 0) )
    ; ( "(no_connect (at 49.53 151.13) (uuid \
         e8352a79-c40d-4cf1-97a7-2bb649ced79a))"
      , Coord (4953, 15113) ) ]
...
let lib_symbols_tests =
  test_list test_lib_symbols
    [ ( {|   (lib_symbols
    (symbol "power:GND" (power) (pin_names (offset 0)) (in_bom yes) (on_board yes)
...
let test_pts =
  create_test pts_expr (fun l1 l2 ->
      List.iter2
        ~f:(fun (Coord (x1, y1)) (Coord (x2, y2)) ->
          assert_equal x1 x2 ; assert_equal y1 y2 )
        l1 l2 )

let pts_tests =
  test_list test_pts
    [ ( "(pts (xy 29.21 68.58) (xy 59.69 68.58))"
      , [Coord (2921, 6858); Coord (5969, 6858)] ) ]
...
let check_global_label (Coord (x1, y1), r1, text1, Size s1, shape1, j1)
    (Coord (x2, y2), r2, text2, Size s2, shape2, j2) =
  assert_equal x1 x2 ~printer:string_of_int ;
  assert_equal y1 y2 ~printer:string_of_int ;
...
let global_label_tests =
  test_list test_global_test
...
let sheet_project_tests =
  test_list test_sheet_project
    [ ( {|
    (instances
      (project "test"
