...
module type EltPainter = sig
  ...
  (** [draw_ellipse center major_radius minor_radius rotation_angle ctx] draws
      an ellipse *)
...
struct
  type wireDesc = { start : coord; stop : coord }
  ...
  type t = { wires : wires; canevas : P.t }

  (* type drawContext = P.t *)

  let create canevas =
    { wires = { wires = []; cons = []; buses = [] }; canevas }
  ...
  let draw_field (Coord (x0, y0)) ((a, b), (c, d)) is_multi refs context
    ...
    let j' =
      ...
      | Orient_V -> if c = 1 || d = -1 then swap_justify j else j
    in
    {
      context with
      canevas = P.paint_text text o' (Coord (x', y')) s j' stl context.canevas;
    }
  ...
  let draw_port ?(kolor = `Black) name ptype justif (Coord (x, y)) (Size l as s)
      ctx =
    ...
    let c =
      ...
      | Orient_V -> Coord (x + (l / 4), y)
    in
    {
      ctx with
      canevas = P.paint_text new_port_name orient c s j NoStyle ctx.canevas;
    }
...
*)
  ...
  let draw_bitmap b ctx =
    let canevas =
      ...
      | Some p, Some s, Some d -> P.paint_image p s d ctx.canevas
      | _ -> ctx.canevas
    in
    { ctx with canevas }
  ...
  let draw_wire l isEntry ctx =
    if isEntry then
      ...
      | [ start; stop ] ->
          {
            canevas =
              P.paint_line ~kolor:`Brown ~width:(Size 2) start stop ctx.canevas;
  ...
  let draw_bus l isEntry ctx =
    if isEntry then
      ...
      | [ start; stop ] ->
          {
            canevas =
              P.paint_line ~kolor:`Blue ~width:(Size 5) start stop ctx.canevas;
            wires = { ctx.wires with cons = start :: stop :: ctx.wires.cons };
          }
      | _ -> raise Not_found (* TODO: find better exception *)
    else
  ...
  let draw_line (l : coord list) (ctx : t) =
    ...
    | start :: (_ :: _ as tail) ->
        ...
        let append_and_memo canevas point =
          let start = !prev in
          prev := point;
          P.paint_line ~kolor:`Black ~width:(Size 2) start point canevas
        in
  ...
  let draw_circle center radius ctx =
    {
      ctx with
      canevas = P.paint_circle ~kolor:`Black center radius ctx.canevas;
    }
  ...
  let draw_ellipse center major_radius minor_radius rotation_angle ctx =
    {
      ctx with
      canevas =
        P.paint_ellipse ~kolor:`Black center major_radius minor_radius
          rotation_angle ctx.canevas;
    }

  let draw_ellipse_arc center major_radius minor_radius rotation_angle
      start_angle end_angle ctx =
    {
      ctx with
      canevas =
        P.paint_ellipse_arc ~kolor:`Black center major_radius minor_radius
          rotation_angle start_angle end_angle ctx.canevas;
    }
  ...
  let draw_arc center start_pt end_pt radius ctx =
    {
      ctx with
      canevas =
        P.paint_arc ~kolor:`Black center start_pt end_pt radius ctx.canevas;
    }
  ...
  let draw_text_line line l (c : t) =
    ...
    | TextLabel t ->
        ...
        let canevas =
          List.fold_left paint_line c.canevas
  ...
  module type OrderedCoord = sig
    val compare : coord -> coord -> int
  end

  module SegmentCutter (O : OrderedCoord) : sig
    val cut_wires :
      wireDesc list -> coord list -> kolor:kolor -> width:size -> P.t -> P.t
  end = struct
    module SegmentSet = Set.Make (struct
      type t = wireDesc

      let compare { start = start1; _ } { start = start2; _ } =
        O.compare start1 start2
    end)
    ...
    let con_in_a_segment c set =
      match
        SegmentSet.find_first_opt (fun { stop; _ } -> O.compare stop c > 0) set
      with
      | None -> None
      | Some ({ start; _ } as seg) ->
          if O.compare start c < 0 then Some seg else None
    ...
    let cut_wire set con =
      ...
      | Some ({ start; stop } as seg) ->
          set |> SegmentSet.remove seg
          |> SegmentSet.add { start; stop = con }
          |> SegmentSet.add { start = con; stop }

    let merge_segment ~set seg =
      SegmentSet.filter
        (fun { start = stt; _ } -> not (point_in_segment stt seg))
        set
      |> SegmentSet.add seg

    let insert_segment set { start; stop } =
      let start, stop =
        if O.compare start stop <= 0 then (start, stop) else (stop, start)
      in
      match (point_in_a_segment start set, point_in_a_segment stop set) with
      | None, None -> merge_segment ~set { start; stop }
      | Some { start = stt; _ }, None ->
          merge_segment ~set { start = stt; stop }
      | None, Some { stop = stp; _ } -> merge_segment ~set { start; stop = stp }
    ...
    let cut_wires seg_list junctions ~kolor ~width canevas =
      ...
      SegmentSet.fold
        (fun { start; stop } canevas ->
          P.paint_line ~kolor ~width start stop canevas)
        split_set canevas
  end

  module VerticalSet = SegmentCutter (struct
    let compare (Coord (xs0, ys0)) (Coord (xs1, ys1)) =
      match Stdlib.compare xs0 xs1 with 0 -> Stdlib.compare ys0 ys1 | c -> c
  end)

  module HorizontalSet = SegmentCutter (struct
    let compare (Coord (xs0, ys0)) (Coord (xs1, ys1)) =
      match Stdlib.compare ys0 ys1 with 0 -> Stdlib.compare xs0 xs1 | c -> c
  end)

  let cut_all_wires junctions wires ~kolor ~width canevas =
    let vertical, horizontal =
      List.partition
        (fun { start = Coord (x1, _); stop = Coord (x2, _) } -> x1 == x2)
        wires
    in
  ...
  let draw_page_frame (Coord (x, y)) ctx =
    ...
    let canevas =
      ctx.canevas |> P.set_canevas_size x y
      |> P.paint_rect
  ...
  let draw_sheet_field name number (Size size as s) (Coord (x, y))
      (Coord (_, dim_y)) ctx =
    let y = if number = 0 then y else y + dim_y + size in
    let canevas =
      P.paint_text name Orient_H (Coord (x, y)) s J_left NoStyle ctx.canevas
    in
    { ctx with canevas }
