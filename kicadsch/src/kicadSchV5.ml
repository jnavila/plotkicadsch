              ...
              | Some d -> (
                  ...
                  | Some origin, Some sym -> (
                      let res =
                        ...
                        | Unique
                            { unitnr = Some m_unitnr; piece = Some m_piece } ->
  ...
  let parse_line line ctx =
    ...
    | DescrContext page_size as c ->
        if String.compare line "$EndDescr" = 0 then
          ...
          { ctx with c; canevas = parse_descr_line line page_size ctx.canevas }
    ...
    | BodyContext -> parse_body_line ctx line
    | WireContext l ->
