...
module MakeSchPainter (P : Painter) :
  ...
  let parse_transfo =
    ...
    create_parse_fun ~name:"Component transformation" ~regexp_str:" %d %d %d %s"
      ~extract_fun:(fun a b c ds ->
        if String.length ds > 0 then
          let d = int_of_string ds in
          if check a && check b && check c && check d then Some (a, b, c, Some d)
          else (
  ...
  let parse_component_line lib (line : string) (comp : componentContext)
      allow_missing canevas : componentContext * EltPainter.t =
    let update_comp comp = (comp, canevas) in
    if String.length line == 0 then (comp, canevas)
    else
      let first = line.[0] in
      match first with
      | 'A' ->
          ...
          @@ parse_F line
               ...
               ~process:(fun (nb, visible, text, o, co, s, j, stl) ->
                 if visible && String.length text > 0 then
                   { comp with
                     fields= {nb; text; o; co; s; j; stl} :: comp.fields }
                 else comp )
      | 'U' ->
          update_comp
          @@ parse_U line
                      ...
                      | Some (refs, m_unitnr) ->
                          ...
                          let canevas', is_multi =
                            EltPainter.modify_canevas
                              (CPainter.plot_comp
                                 (KicadLibParserV1.get_comp_lib lib)
                                 sym m_unitnr origin transfo allow_missing )
                              canevas
                          in
                          let draw =
                            EltPainter.draw_field origin transfo is_multi refs
                          in
    ...
    if String.compare line "$Comp" = 0 then
      ...
      parse_descr_header line
        ...
        ~process:(fun (_, (Coord (x, y) as f_left)) ->
          { ctx with
            c= DescrContext (Coord (x - 4000, y - 100))
      ...
      let lab : label option =
        parse_text_line line ~onerror:(fun () -> None) ~process:(fun l -> Some l)
      in
  ...
  let parse_descr_line line c canevas =
    parse_descr_body line
      ...
      ~process:(fun (field, content) ->
        EltPainter.draw_title_field c field content canevas )
  ...
  let parse_bitmap_line line b =
    if starts_with line "Pos" then
      { b with
        pos=
          parse_bm_pos line ~onerror:(fun () -> b.pos) ~process:(fun c -> Some c)
      }
    ...
    else if starts_with line "Data" then {b with data= Some (Buffer.create 1000)}
    else (append_bm_line b.data line ; b)

  let write_revision c ctx = EltPainter.write_revision c ctx.rev ctx.canevas
  ...
  let parse_line line ctx =
    ...
    | ComponentContext comp ->
        if String.compare line "$EndComp" = 0 then {ctx with c= BodyContext}
          ...
          let comp, canevas =
            parse_component_line ctx.lib line comp ctx.allow_missing_component
              ctx.canevas
          in
        ...
        parse_wire_line line
          ...
          ~process:(fun (start, stop) ->
            let canevas =
              match l with
              | Bus ->
                  EltPainter.draw_bus [start; stop] false ctx.canevas
              | BusEntry ->
                  EltPainter.draw_bus [start; stop] true ctx.canevas
              | Wire ->
                  EltPainter.draw_wire [start; stop] false ctx.canevas
              | WireEntry ->
                  EltPainter.draw_wire [start; stop] true ctx.canevas
              | Line ->
                  EltPainter.draw_line [start; stop] ctx.canevas
            in
            {ctx with c= BodyContext; canevas} )
    | SheetContext sc ->
    ...
    | TextContext sc -> (
      ...
      | Some v ->
          { ctx with
            c= BodyContext
          ; canevas= EltPainter.draw_text_line line v ctx.canevas } )
    | BitmapContext b ->
        if String.compare line "$EndBitmap" = 0 then
          { ctx with
            c= BodyContext
          ; canevas= EltPainter.draw_bitmap b ctx.canevas }
        else
  ...
  let trim_cr l =
    if String.ends_with ~suffix:"\r" l then String.sub l 0 (String.length l - 1)
    else l
  ...
  let add_lib content ctxt =
    ...
    let lib =
      List.fold_left
        (fun c l -> KicadLibParserV1.append_lib (trim_cr l) c)
        ctxt.lib lines
    in
    {ctxt with lib}
