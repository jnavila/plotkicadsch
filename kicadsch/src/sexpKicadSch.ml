...
let uuid_args =
  string ~escaped:false >>= fun s ->
  match Uuidm.of_string s with
  | Some u -> return u
  | None -> failwith "Not a uuid"
...
let paper_size_args =
  ...
  let (Coord (x, y) as c) =
    ...
    | "User" -> (
        match size with
        | Some coords -> coords
        | None -> failwith "User paper size requires a size")
    ...
    | s -> failwith ("unknown paper size " ^ s)
  in
...
let fill_expr : fill decoder =
  field "fill"
    (fields
       ...
       [
         ( "type",
           fill_type_atom >>| fun fill_t arg ->
           { arg with fill_type = Some fill_t } );
         ( "color",
           kolor_args >>| fun color arg -> { arg with kolor = Some color } );
       ])

let horizontal_justify_atom =
  atom >>= function
  | "left" -> return J_left
  | "right" -> return J_right
...
let text_args =
  text_gen_args >>| fun (coords, text, size, _rot, _justify) ->
...
let pin_type_atom =
  atom >>| function
  | "bidirectional" | "passive" | "input" | "output" | "power_in" | "power_out"
  | "open_collector" | "open_drain" | "open_emitter" | "emitter_follower"
  | "source_follower" | "unconnected" | "no_connect" | "tri_state"
  | "unspecified" ->
      true
...
let pin_shape_atom =
  atom >>| function
  | "none" | "line" | "inverted" | "clock" | "inverted_clk" | "input_low"
  | "clock_low" | "falling_edge" | "non_logic" ->
      true
...
let ellipse_common_args =
  ...
  let* major_radius =
    field "major_radius" float >>| fun r ->
    Float.to_int (Float.round (r *. 100.0))
  in
  let* minor_radius =
    field "minor_radius" float >>| fun r ->
    Float.to_int (Float.round (r *. 100.0))
  in
  let+ rotation_angle =
...
let bus_entry_args =
  let* position = at_expr in
  let* size =
    field "size" (tuple2 float float) >>| fun (s1, s2) ->
    (mm_size s1, mm_size s2)
  in
...
let sch_symbol_args =
  ...
  let+ lib_sym =
    fields
      ...
      [
        ( "at",
          pin_at_coord_args >>| fun (pos, rot) args -> { args with pos; rot } );
        ...
        ( "property",
          property_args >>| fun prop args ->
          { args with properties = prop :: args.properties } );
...
let image_args =
  fields
    ...
    [
      ( "at",
        pin_at_coord_args >>| fun (apos, _) (args : bitmapContext) ->
        { args with pos = Some apos } );
      ("uuid", atom >>| fun _ args -> args);
