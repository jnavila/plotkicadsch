...
let dist_expr s = field s float >>| fun s -> wx_size s

let length_expr = dist_expr "length"
...
let gen_uuid_expr s = field s uuid_args
...
let paper_size_args =
  ...
  let (Coord (x, y) as c) =
    match s with
    | "A5" ->
        Coord (mm_size 210., mm_size 148.)
    | "A4" ->
        Coord (mm_size 297., mm_size 210.)
    | "A3" ->
        Coord (mm_size 420., mm_size 297.)
    | "A2" ->
        Coord (mm_size 594., mm_size 420.)
    | "A1" ->
        Coord (mm_size 841., mm_size 594.)
    | "A0" ->
        Coord (mm_size 1189., mm_size 841.)
    | "A" ->
        Coord (wx_size 11000., wx_size 8500.)
    | "B" ->
        Coord (wx_size 17000., wx_size 11000.)
    | "C" ->
        Coord (wx_size 22000., wx_size 17000.)
    | "D" ->
        Coord (wx_size 34000., wx_size 22000.)
    | "E" ->
        Coord (wx_size 44000., wx_size 34000.)
    (*  | "GERBER" ->  coords *)
    | "User" -> (
      match size with
      | Some coords ->
          coords
      | None ->
          failwith "User paper size requires a size" )
    | "USLetter" ->
        Coord (wx_size 11000., wx_size 8500.)
    | "USLegal" ->
        Coord (wx_size 14000., wx_size 8500.)
    | "USLedger" ->
        Coord (wx_size 17000., wx_size 11000.)
    | s ->
        failwith ("unknown paper size " ^ s)
  in
  match p with Some _ -> Coord (y, x) | None -> c

let paper_expr = field "paper" paper_size_args

let xy_expr = field "xy" coords

let pts_expr = field "pts" (repeat_full_list xy_expr)

let make_rel (Coord (x, y)) = RelCoord (x, y)

let make_rel_pts pts = List.map ~f:make_rel pts

let string_expr s = field s (string ~escaped:false)

let int_expr s = field s int

let float_expr s = field s float
...
type fill_type = No_fill | Background_fill | Outline_fill
...
type fill = {fill_type: fill_type option; kolor: kolor option}

let fill_expr : fill decoder =
  field "fill"
    (fields
       ~default:({fill_type= None; kolor= None} : fill)
       [ ( "type"
         , fill_type_atom >>| fun fill_t arg -> {arg with fill_type= Some fill_t}
         )
...
type justification = {horiz: justify option; vert: justify option}

let justify_args =
  let+ justif =
    tuple2 (maybe horizontal_justify_atom) (maybe vertical_justify_atom)
  in
...
let opt_to_bool = function Some _ -> true | None -> false
...
let bool_or_tag name = yesno_expr name |+> (tag name >>| fun _ -> true)
...
type effects = {font: font_def; justify: justification option; hide: bool}
...
let fontsize_of_effect e =
  let (Coord (_x, y)) = e.font.size in
  y
...
let line_style_expr = field "type" style_args
...
let width_expr =
  float_expr "width"
  >>| fun w ->
  let width = wx_size w in
  Width width

let optional_width_expr = maybe width_expr

type stroke = {width: width option; kolor: kolor option; style: string option}
...
let stroke_expr = field "stroke" stroke_args

let optional_stroke_to_width = function
  | None | Some {width= None; _} ->
      10
  | Some {width= Some (Width w); _} ->
      w
...
let field_build {name; value; id; at; rot; effects} =
  ...
  let s, j, stl, hide =
    match effects with
    | None ->
        (Size default_width, J_left, NoStyle, false)
    | Some ef ->
        let j = justify_of_effect ef in
        let (Coord (_, y)) = ef.font.size in
        let s = Size y in
        let style =
          match (ef.font.italic, ef.font.bold) with
          | true, false ->
              Italic
          | false, true ->
              Bold
...
let text_args =
  text_gen_args
  >>| fun (coords, text, size, _rot, _justify) ->
...
let polyline_expr = field "polyline" polyline_args

let start_point_expr = field "start" coords

let end_point_expr = field "end" coords
...
let rectangle_expr = field "rectangle" rectangle_args

let rectangle_prim_expr = field "rectangle" rectangle_prim_args
...
let pin_type_atom =
  ...
  | "unspecified" ->
      true
  | s ->
      failwith (Printf.sprintf "no match for pin type (%s)" s)
...
let pin_shape_atom =
  ...
  | "non_logic" ->
      true
  | s ->
      failwith (Printf.sprintf "no match for pin shape (%s)" s)
...
let pin_args =
  ...
  let name =
    let (Coord (x, _)) = name_effect.font.size in
    (name_str, Size x)
  in
  let number =
    let (Coord (x, _)) = number_effect.font.size in
    (number_str, Size x)
  in
...
let radius_expr = field "radius" radius_args

let mid_point_expr = field "mid" coords >>| make_rel

let arc_center_radius_from_rel_points (RelCoord (ax, ay)) (RelCoord (bx, by))
    (RelCoord (cx, cy)) =
  ...
  if Float.abs d < 1e-3 then None
    ...
    let r =
      Float.sqrt (((ux -. ax) *. (ux -. ax)) +. ((uy -. ay) *. (uy -. ay)))
    in
    Some
      ( RelCoord (Float.to_int (Float.round ux), Float.to_int (Float.round uy))
    ...
    | None -> (
      ...
      | Some mid_pt -> (
        ...
        | Some (c, r) ->
            (c, r)
        | None ->
            failwith "arc: start, mid, end points are collinear" )
      | None ->
          failwith "arc: either radius or mid point must be specified" )
  in
  Arc {sp; ep; s; radius; center}

let arc_expr = field "arc" arc_args
...
let bezier_expr = field "gr_curve" bezier_args

let center_expr = field "center" coords >>| make_rel
...
let circle_expr = field "circle" circle_args
...
let ellipse_common_args =
  ...
  let* major_radius =
    field "major_radius" float
    >>| fun r -> Float.to_int (Float.round (r *. 100.0))
  in
  let* minor_radius =
    field "minor_radius" float
    >>| fun r -> Float.to_int (Float.round (r *. 100.0))
  in
  let+ rotation_angle =
    field "rotation_angle" float >>| Float.round >>| int_of_float
  in
...
let sch_ellipse_arc_args =
  let* center, ma, mi, rot = ellipse_common_args in
  let* start_angle =
    field "start_angle" float >>| Float.round >>| int_of_float
  in
...
let unit_args =
  ...
  | _style :: part_str :: _ ->
      let parts = int_of_string part_str in
      List.map ~f:(fun prim -> {parts; prim}) graphics
  | _ ->
      failwith ("malformed unit: " ^ name)

let unit_expr = field "symbol" unit_args

let unit_peek expr = peek expr >>= function Some _ -> return () | _ -> error

let pin_number_hide_expr =
  maybe_with_default false @@ (field "pin_numbers" skip >>= fun _ -> return true)

let offset_expr = maybe_with_default 0 @@ dist_expr "offset"

let pin_names_expr =
  maybe_with_default true
  @@ (field "pin_names" (offset_expr <*> maybe skip) >>= fun _ -> return false)
...
let symbol_args =
  ...
  let+ units =
    repeat_full_list
      (unit_expr |+> field "embedded_fonts" (skip >>| fun _ -> []))
  in
  let graph = List.flatten units in
  { names= [name]
  ; draw_pnum= not hide_pin_numbers
  ; draw_pname
  ; multi= false
  ; graph }
...
let bus_entry_args =
  let* position = at_expr in
  let* size =
    field "size" (tuple2 float float)
    >>| fun (s1, s2) -> (mm_size s1, mm_size s2)
  in
    ...
    fields
      ...
      ; ( "mirror"
        , atom
          ...
          match axis with
          | "x" ->
              {args with mirror_x= true}
          | "y" ->
              {args with mirror_y= true}
          | _ ->
              failwith "unknown mirror axis" )
      ; ("uuid", atom >>| fun _ args -> args)
      ; ( "property"
        , property_args
          >>| fun prop args -> {args with properties= prop :: args.properties}
        )
...
let image_args =
  fields
    ...
    [ ( "at"
      , pin_at_coord_args
        >>| fun (apos, _) (args : bitmapContext) -> {args with pos= Some apos}
      )
    ; ("uuid", atom >>| fun _ args -> args)
    ; ("scale", float >>| fun scale args -> {args with scale= Some scale})
    ; ( "data"
      , repeat1_full_list (string ~escaped:false)
        >>| fun (strings : string list) args ->
        let d = Buffer.create 1000 in
        List.iter ~f:(Buffer.add_string d) strings ;
        match Base64.decode (Buffer.contents d) with
        | Ok st ->
            let databuf = Buffer.create (String.length st) in
            Buffer.add_string databuf st ;
            {args with data= Some databuf}
        | Error (`Msg msg) ->
            Format.print_string msg ; args ) ]
...
let sheet_instances_args =
  repeat1_full_list sheet_project_instance_expr >>| fun _ -> 0
    ...
    repeat_list
      ~until:
        (unit_peek
           ( sheet_pin_expr
           >>| (fun _ -> ())
           |+> (sheet_instances_expr >>| fun _ -> ()) ) )
      property_expr
  in
  let* hierarchical_pins =
    repeat_list ~until:(unit_peek sheet_instances_expr) sheet_pin_expr
  in
...
let title_block_args =
  ...
  let+ comments =
    repeat_full_list (field "comment" (int <*> string ~escaped:false))
  in
...
let title_block_v8_args =
  let* title =
    maybe (field "title" (string ~escaped:false)) >>| Option.value ~default:""
  in
  let* date =
    maybe (field "date" (string ~escaped:false)) >>| Option.value ~default:""
  in
  let* rev =
    maybe (field "rev" (string ~escaped:false)) >>| Option.value ~default:""
  in
  let* company =
    maybe (field "company" (string ~escaped:false)) >>| Option.value ~default:""
  in
  let+ comments =
    repeat_full_list (field "comment" (int <*> string ~escaped:false))
  in
...
let sheet_instances_args_v8 =
  repeat1_full_list sheet_project_instance_expr_v8 >>| fun _ -> 0
...
let sheet_args_v8 =
  ...
  in
  let* hierarchical_pins =
    repeat_list ~until:(unit_peek sheet_instances_expr_v8) sheet_pin_expr
  in
...
let sch_arc_center_radius (Coord (ax, ay)) (Coord (bx, by)) (Coord (cx, cy)) =
  ...
  if Float.abs d < 1e-3 then None
    ...
    in
    Some
      ( Coord (Float.to_int (Float.round ux), Float.to_int (Float.round uy))
      , Float.to_int (Float.round r) )
...
let sch_circle_args =
  let* center = field "center" coords in
  let* radius =
    field "radius" float >>| fun r -> Float.to_int (Float.round (r *. 100.0))
  in
  let* _stroke = maybe stroke_expr in
  let* _fill = maybe fill_expr in
  let* _uuid = maybe uuid_expr in
...
let sch_bezier_args =
  ...
  match pts with
  | [p0; p1; p2; p3] ->
      sch_bezier_sample p0 p1 p2 p3 16
  | _ ->
      pts (* fallback: draw the control polygon *)
