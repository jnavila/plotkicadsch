...
let make rootname relative =
  (module struct
    ...
    let get_content filename =
      let filepath =
        String.concat ~sep:Filename.dir_sep (rootname :: filename)
      in
      try%lwt Lwt_io.with_file ~mode:Lwt_io.input filepath Lwt_io.read
      with _ -> Lwt.return ""
    ...
    let list_files pattern =
      ...
      let file_list =
        Lwt_list.map_s
          (fun filename ->
            let filename =
              String.sub filename ~pos:rootlength
