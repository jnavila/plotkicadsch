...
(** [doit fs_from fs_to filename differ textdiff libs keep colors allow_missing
     relative_path] performs the diff of [filename] from [relative_path] if
    present between [fs_from] and [fs_to] using strategy [differ] and using
    common [libs] and [colors] scheme. If [textdiff], then a text diff is shown
    when no visual diff, if [keep] then the diff file isn't removed after *)
