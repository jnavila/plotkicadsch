...
type content =
  [ `Polyline | `Text | `Svg | `Rect | `Circle | `Ellipse | `Path | `Image ]
...
type t = {
  d : dim;
  c : content elt list;
  colors : diff_colors option;
  zone_color : string option;
}
...
let color_of_kolor k { colors; zone_color; _ } =
  ...
  | `New -> plain new_ver
  | `ForeGround -> plain fg
  | `Zone -> ( match zone_color with None -> `None | Some c -> plain c)
      ...
      ellipse
        ~a:
          [
            ...
            a_transform
              [ `Rotate ((rot, None), Some (float_of_int cx, float_of_int cy)) ];
            a_fill (color_of_kolor fill ctxt);
            a_stroke_width (10., Some `Px);
            a_stroke (color_of_kolor kolor ctxt);
          ]
...
let paint_ellipse_arc ?(kolor = `Black) ?(fill = `NoColor) (Coord (cx, cy))
    major_radius minor_radius rotation_angle start_angle end_angle
    ({ c; _ } as ctxt) =
    ...
    c =
      path
        ~a:
          [
            a_d
              (Printf.sprintf "M%d,%d A%d,%d %d %d,%d %d,%d" (Float.to_int sx)
                 (Float.to_int sy) major_radius minor_radius rotation_angle
...
let paint_zone (Coord (x, y)) (Coord (dim_x, dim_y)) ({ c; _ } as ctxt) =
  ...
  let render =
    if fill_color == `None then [ a_style "fill-opacity: 0;" ]
    else [ a_fill (color_of_kolor `Zone ctxt); a_style "fill-opacity: 0.1;" ]
  in
  {
    ctxt with
    c =
...
let get_color_context colors zone_color =
  { d = (0, 0); c = []; colors; zone_color }

let new_from { colors; zone_color; _ } =
  { d = (0, 0); c = []; colors; zone_color }
