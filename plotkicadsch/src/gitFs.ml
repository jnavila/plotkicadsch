...
let make commitish relative_path =
  (module struct
    ...
    let rev_parse r =
      ...
      try
        Lwt.return @@ Store.Hash.of_hex
        @@ String.sub ~pos:0 s ~len:(min 40 (String.length s))
    ...
    let with_path path action =
      ...
      with
      | None -> Lwt.fail (PathNotFound path)
      | Some sha -> (
      ...
      let recurse_tree =
       fun entry ->
        ...
        | Ok (Git.Value.Tree t) ->
            ...
            let subdir_files =
              List.map ~f:(fun (name, hash) -> (rename name, hash)) subdir
            in
            Lwt.return subdir_files
        | Ok _ -> Lwt.fail (InternalGitError "impossible case")
      in
