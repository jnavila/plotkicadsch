...
let internal_diff (d : string) (c : SvgPainter.diff_colors option)
  ...
  (module struct
    ...
    let plot_elt style out_ctx (arg : ListPainter.t) =
      ...
      let kolor =
        match style with Theirs -> `Old | Ours -> `New | Idem -> `ForeGround
      in
      match arg with
    ...
    let text_bbox text o c s j =
      ...
      match o with
      | Orient_H ->
          BoundingBox.create_from_rect
            (Coord (x + shift, y))
            (Coord (sz * len / 2, sz / 2))
      | Orient_V ->
          BoundingBox.create_from_rect
            (Coord (x, y - (sz * len / 2) + shift))
            (Coord (sz / 2, sz * len / 2))
    ...
    let rec aggregate rect rect_list =
      let result, remaining =
        List.fold_left ~f:dispatch_rect ~init:(rect, []) rect_list
      in
      if Int.equal (List.length remaining) (List.length rect_list) then
        (result, remaining)
      else aggregate result remaining
    ...
    let refine_segments (Coord (x1, y1), _) (Coord (x1', y1'), Coord (x2', y2'))
        =
      if Int.compare x1 x1' == 0 then Int.compare y1 y1' * Int.compare x1 x2'
      else Int.compare y1 y2'

    let compare s1 s2 : int =
      let s1_r = elt_rect s1 and s2_r = elt_rect s2 in
    ...
    let draw_difftotal ~prev ~next out_canevas =
      let rec rec_draw_difftotal ~prev ~next (idem, theirs, ours, outc)
          diff_list =
        ...
        | p :: pl, n :: nl ->
            ...
            if comp == 0 then
              rec_draw_difftotal ~prev:pl ~next:nl
                (plot_elt Idem idem p, theirs, ours, outc)
                diff_list
            else if comp < 0 then
              rec_draw_difftotal ~prev:pl ~next
                (idem, plot_elt Theirs theirs p, ours, outc)
                (r p :: diff_list)
            else
              rec_draw_difftotal ~prev ~next:nl
