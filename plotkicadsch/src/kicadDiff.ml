...
let doc = function TrueFS s -> "file system " ^ s | GitFS s -> "Git rev " ^ s
...
let fs_mod s r =
  let rel_path =
    Option.bind r (fun rel_path ->
        if
          String.length rel_path > 1
          && String.equal (String.sub rel_path ~pos:0 ~len:2) "./"
        then
          begin if String.length rel_path == 2 then None
          else
...
let is_suffix ~suffix s =
  ...
  && String.equal
       (String.sub s ~pos:(String.length s - suff_length) ~len:suff_length)
       suffix
...
module FSPainter (S : SchPainter) (F : Simple_FS) : sig
  ...
  let process_file initctx filename =
    ...
    F.get_content filename >|= S.parse_sheet init >|= S.output_context

  let find_libs () =
    F.list_files (is_suffix ~suffix:"-cache.lib") >|= List.map ~f:fst

  let read_libs (initial_ctx : S.schContext) (lib_list : string list list) :
      S.schContext Lwt.t =
...
module PathCompare = struct
  ...
  let rec sl_compare l1 l2 =
    match (l1, l2) with
    | name1 :: tl1, name2 :: tl2 ->
        let res = String.compare name1 name2 in
        if res == 0 then sl_compare tl1 tl2 else res
    | _h :: _t, [] -> 1
    | [], _h :: _t -> -1
    | [], [] -> 0
...
let diff_cmd f t filename =
  ...
  | TrueFS fc, TrueFS tc ->
      ( "diff",
        [| fc ^ Filename.dir_sep ^ filename; tc ^ Filename.dir_sep ^ filename |]
      )

let doit from_fs to_fs file_to_diff differ textdiff libs keep colors zone_color
    allow_missing_component relative_path =
  let module_d =
    match differ with
    | Image_Diff -> (module ImageDiff : Differ)
    | Internal s -> InternalDiff.internal_diff s colors zone_color
  in
  ...
  let preload_libs desc =
    Lwt_list.fold_left_s
      ...
      (D.S.initial_context ~allow_missing_component desc)
      libs
  in
  let from_init_ctx =
    FromP.context_from @@ preload_libs (First (doc from_fs))
  in
  ...
  let compare_one filename =
    ...
    | false ->
        if textdiff then
          let cmd, args =
            diff_cmd F.label T.label @@ String.concat ~sep:"/" filename
          in
  ...
  let catch_errors =
    Lwt.catch
      (fun _ ->
        ...
        >>= fun _ -> compare_all)
      (function
        | GitFs.InternalGitError s -> Lwt_io.printf "Git Exception: %s\n" s
